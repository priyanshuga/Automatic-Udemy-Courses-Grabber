...
def real_disc(page):
    ...
    soup = BeautifulSoup(r.content, 'html.parser')
    udd = soup.find_all('div','add-links no-effect clearfix')
    ...
    data = content[0:13]
    for index, i in enumerate(udd):
        title=index.img['title']
        url2=index.a['href']
        sys.stdout.write("\rLOADING URLS: " +
                         animation[index % len(animation)])
        sys.stdout.flush()
        ...
        if udd[index].text.replace('\n', '') == 'Udemy':
            ...
            title=soup1.find('title').text.replace(
                ' Udemy Coupon - Real Discount', '')
            links_ls.append(title + '||' + soup1.find('div',
                                                      class_ = 'col-sm-6 col-xs-6 letshover').a['href'])
    return links_ls
...
def udemy_coupons_me(page):
    ...
    print("\nPage Number", page)
    r=requests.get(UDEMYCOUPONS, headers=head, verify=False)
    soup=BeautifulSoup(r.content, 'html.parser')
    all=soup.find_all('div', 'td-module-thumb')
    if page == 1:
        all=all[0:20]
    for index, items in enumerate(all):
        title=items.a['title']
        ...
        try:
            ll=soup1.find(
                'span', class_='td_text_highlight_marker_green td_text_highlight_marker').a['href']
...
def jojocoupons(page):
    ...
    soup=BeautifulSoup(r.content, 'html.parser')
    all=soup.find_all(
        'h2', class_='font130 mt0 mb10 mobfont110 lineheight20')
    for index, items in enumerate(all):
        title=items.text
        url2=items.a['href']
        r2=requests.get(url2, headers=head, verify=False)
